...
const TAB_TYPES = [
  { value: 'admin', labelKey: 'roles.admin', icon: <AdminIcon fontSize="small" /> },
  { value: 'companyUser', labelKey: 'roles.companyUser', icon: <CompanyIcon fontSize="small" /> },
  { value: 'vendor', labelKey: 'roles.vendor', icon: <VendorIcon fontSize="small" /> },
];
...
export default function IntegrationPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await apiClient.post('/integration', { type: currentType, script: script.trim(), token: token.trim() });
    ...
    <DashboardLayout>
      <Box>
        ...
        >
          ...
          >
            {TAB_TYPES.map((tab) => (
              <Tab key={tab.value} icon={tab.icon} iconPosition="start" label={t(tab.labelKey)} />
            ))}
          </Tabs>
                  ...
                  <TabPanel key={tab.value} value={activeTab} index={index}>
                    {integrations.find((i) => i.type === tab.value) ? (
                      ...
                      >
                        ...
                        >
                          {integrations.find((i) => i.type === tab.value)?.script}
                        </Box>
                        {integrations.find((i) => i.type === tab.value)?.token && (
                          <>
                        ...
                        <Button
                          variant="outlined"
                          color="error"
                          startIcon={submitting ? <CircularProgress size={18} /> : <DeleteIcon />}
                      ...
                      >
                        ...
                        >
                          {submitting ? t('integration.adding') : t('integration.addIntegration')}
                        </Button>
                      </Box>
                    )}
                  </TabPanel>
